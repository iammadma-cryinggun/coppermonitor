...
print("="*80)
print("锡(Sn) - 样本外测试（优化后参数）".center(80))
print("="*80)
...
# 锡的最优参数
params = [12, 31, 15, 39, 14, 19, 53, 1.40, 12, 26, 44, 92, 0.0273]

print(f"\n锡的最优参数:")
print(f"  EMA: (12, 31)")
print(f"  MACD: (15, 39, 14)")
print(f"  RSI: 周期19 阈值53")
print(f"  Ratio: 阈值1.40")
print(f"  STC: 周期12 (26, 44) 卖出92")
print(f"  止损: 2.73%")
...
status_win = "[OK]" if win_test >= 40 else "[WARN]"
...
print("\n" + "="*80)
print("对比：优化参数 vs 通用参数".center(80))
print("="*80)

print(f"\n{'指标':<12} | {'通用参数':<18} | {'优化参数':<18} | {'改进':<15}")
print("-"*80)

print(f"{'样本内收益率':<12} | {59.58:>15.2f}% | {ret_train:>15.2f}% | {ret_train-59.58:>+13.2f}%")
print(f"{'样本外收益率':<12} | {70.25:>15.2f}% | {ret_test:>15.2f}% | {ret_test-70.25:>+13.2f}%")
print(f"{'样本外胜率':<12} | {54.5:>15.1f}% | {win_test:>15.1f}% | {win_test-54.5:>+13.1f}%")

print("-"*80)

print("\n" + "="*80)
if ret_test > 50 and win_test >= 45:
    print("[EXCELLENT] 锡通过了优化参数样本外测试！")
    print("            优化参数进一步提升表现，锡是FTT策略的最佳标的！")
elif ret_test > 30 and win_test >= 35:
    print("[GOOD] 锡优化参数样本外表现良好")
    print("      略优于通用参数，优化有价值")
else:
    print("[WARN] 优化参数样本外表现不如预期")
    print("       可能存在轻微过拟合，建议使用通用参数")
print("="*80)
