...
class ChinaFuturesFetcher:
    ...
    FUTURES_MAPPING = {
        # === 能源中心 (INE) ===
        # ⚠️ 注意: 原油在新浪财经的代码需要特殊处理，此处使用 SC0
        # 如果与PVC冲突，请使用不同的数据源或手动指定正确的代码
        'SC': {'name': '原油', 'sina_code': 'SC0', 'exchange': 'INE', 'unit': '元/桶'},
    ...
    def calculate_historical_volatility(self, code: str, window: int = 20) -> Optional[float]:
        ...
        try:
            ...
            # 修复：4小时K线的年化因子
            # 一年约252个交易日，每天6个4小时K线（01:00, 09:00, 13:00, 21:00 + 可能有夜盘）
            # 约为 252 * 6 = 1512 个4小时K线/年
            periods_per_year = 252 * 6  # 4小时K线年化因子
