...
def save_signal_log(signal, status='pending', actual_price=None, actual_action=None):
    ...
    # 信号去重：检查是否已存在相同K线时间的相同类型信号
    signal_datetime = signal.get('datetime')
    signal_type = signal.get('signal_type')

    if signal_datetime and signal_type:
        for existing_log in logs:
            if (existing_log.get('signal_datetime') == signal_datetime and
                existing_log.get('signal', {}).get('signal_type') == signal_type):
                logger.info(f"[去重] 跳过重复信号: {signal_datetime} - {signal_type}")
                return
