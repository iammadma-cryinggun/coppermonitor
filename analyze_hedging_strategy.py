...
def analyze_hedging_strategy():
    """分析对冲策略"""

    print("=" * 120)
    print("PTA对冲策略分析".center(120))
    print("=" * 120)
    ...
    print(f"\n【策略说明】")
    print(f"  当只出现做多信号：开多仓")
    print(f"  当只出现做空信号：开空仓")
    print(f"  当同时出现多空信号：对冲（同时开多仓+空仓）")
    ...
    for i in range(200, len(df)):
        ...
        if long_signal and short_signal and position_long is None and position_short is None:
            # 对冲：同时开多空
            print(f"\n[{current['datetime']}] 检测到对冲信号！")

            # 开多仓
            signal_type_long = 'sniper_long' if sniper_long else 'chase_long'
            entry_price_long = current['close']
            stop_loss_long = entry_price_long * (1 - STOP_LOSS_PCT)

            # 开空仓
            signal_type_short = 'sniper_short' if sniper_short else 'chase_short'
            entry_price_short = current['close']
            stop_loss_short = entry_price_short * (1 + STOP_LOSS_PCT)

            # 计算手数（对冲时各用一半资金）
            contract_value = current['close'] * CONTRACT_SIZE
            margin_per_contract = contract_value * MARGIN_RATE

            # 多仓手数
            potential_loss_long = (entry_price_long - stop_loss_long) * CONTRACT_SIZE
            max_contracts_risk_long = int((capital * 0.30) / potential_loss_long)  # 30%资金
            max_contracts_margin_long = int((capital * 0.40) / margin_per_contract)  # 40%保证金
            contracts_long = min(max_contracts_risk_long, max_contracts_margin_long)

            # 空仓手数
            potential_loss_short = (stop_loss_short - entry_price_short) * CONTRACT_SIZE
            max_contracts_risk_short = int((capital * 0.30) / potential_loss_short)  # 30%资金
            max_contracts_margin_short = int((capital * 0.40) / margin_per_contract)  # 40%保证金
            contracts_short = min(max_contracts_risk_short, max_contracts_margin_short)
            ...
            if contracts_long > 0 and contracts_short > 0:
                ...
                print(f"  开多仓: {contracts_long}手 @ {entry_price_long:.2f}")
                print(f"  开空仓: {contracts_short}手 @ {entry_price_short:.2f}")

        elif long_signal and position_long is None:
            # 只做多
            ...
            margin_per_contract = contract_value * MARGIN_RATE
            potential_loss = (entry_price - stop_loss) * CONTRACT_SIZE

            max_contracts_risk = int((capital * 0.15) / potential_loss)
            max_contracts_margin = int((capital * 0.80) / margin_per_contract)
            contracts = min(max_contracts_risk, max_contracts_margin)
        ...
        elif short_signal and position_short is None:
            # 只做空
            ...
            contract_value = entry_price * CONTRACT_SIZE
            margin_per_contract = contract_value * MARGIN_RATE
            potential_loss = (stop_loss - entry_price) * CONTRACT_SIZE
    ...
    if len(hedge_trades) > 0:
        ...
        print(f"  盈亏: {hedge_trades['pnl'].sum():+,.2f}元")
