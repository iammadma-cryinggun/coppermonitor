...
STRATEGY_REPO = {
    ...
    'TREND': {
        ...
        'vol_filter': 1.0,           # 必须放量
        'stop_loss': 2.0,            # 宽止损
        'take_profit': 999,          # 无固定止盈 (移动止损)
        'use_ma_filter': True,       # 严格顺均线
    },
...
# ========================================================
# 3. 肌肉1：动态指标计算 (Dynamic Calculation)
# ========================================================
def calculate_indicators(df, cci_period=14, stc_length=10, stc_fast=23, stc_slow=50):
    """
    根据策略参数，动态计算指标
    SNIPER用14周期CCI，TREND用20周期CCI
    """
    # 1. 计算 CCI
    ...
    # 2. 计算 STC (Schaff Trend Cycle)
    # MACD Line
    ema_fast = df['close'].ewm(span=stc_fast, adjust=False).mean()
    ema_slow = df['close'].ewm(span=stc_slow, adjust=False).mean()
    macd = ema_fast - ema_slow

    # %K of MACD
    stc_min = macd.rolling(window=stc_length).min()
    stc_max = macd.rolling(window=stc_length).max()
    stc_k = 100 * (macd - stc_min) / (stc_max - stc_min)

    # %D of MACD (Double Smoothed) -> STC
    stc_d = stc_k.ewm(span=stc_length//2, adjust=False).mean()
    df['stc'] = stc_d.ewm(span=stc_length//2, adjust=False).mean()
...
def auto_run_strategy(df, symbol):
    ...
    # B. 准备数据 (根据参数动态计算指标)
    print(f"  [准备] 动态计算指标，CCI周期:{params['cci_period']}")
    ...
    for i in range(len(df) - 1):
        ...
        if position == 0:
            ...
            if params['short_enabled']:
                # SNIPER_SHORT: 高位死叉
                # TREND: 跌破中位
                if regime == 'SNIPER_SHORT':
                    cond_stc = (df.iloc[i-1]['stc'] > params['stc_entry']) if i > 0 else False
                    cond_stc_slope = current['stc_slope'] < params['stc_slope']
                else:  # TREND
                    cond_stc = current['stc'] < 0 and current['stc_slope'] < params['stc_slope']

                vol_ok = current['vol_ratio'] > params['vol_filter']

                # CCI: 从高位回落
                cond_cci = (df.iloc[i-1]['cci'] > params['cci_entry']) if i > 0 else False
                cond_cci_confirmed = current['cci'] < current['cci_prev']

                if cond_stc and cond_stc_slope and vol_ok and cond_cci and cond_cci_confirmed:
            ...
            if params['long_enabled']:
                # SNIPER_LONG: 低位金叉
                # TREND: 突破中位
                if regime == 'SNIPER_LONG':
                    cond_stc = (df.iloc[i-1]['stc'] < -params['stc_entry']) if i > 0 else False
                    cond_stc_slope = current['stc_slope'] > abs(params['stc_slope'])
                else:  # TREND
                    cond_stc = current['stc'] > 0 and current['stc_slope'] > abs(params['stc_slope'])
                ...
                if cond_stc and cond_stc_slope and vol_ok and is_bullish and cond_cci and cond_cci_confirmed and trend_ok:
                    entry_price = next_day['open'] * (1 + SLIPPAGE_RATE)
                    entry_atr = current['atr']
                    stop_distance = entry_atr * params['stop_loss']
...
if __name__ == "__main__":
    print("=" * 100)
    print("全自动 S.C.C. 智能交易系统 v2.0 - 完全闭环版".center(100))
