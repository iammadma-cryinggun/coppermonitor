"""
论文因子完整测试 - 包含所有4个因子及其组合
因子清单：
1. 隔夜缺口因子 (GapZ10) - Rank IC: 0.0793
2. 缺口接受因子 (GapAccept) - Rank IC: 0.0744
3. 趋势质量因子 (TrendQuality) - Rank IC: 0.0590
4. 有序趋势因子 (OrderlyTrend) - Rank IC: 0.0465
"""
import sys
...
def main():
    print("=" * 140)
    print("论文因子完整测试 - 4个因子及其组合".center(140))
    print("=" * 140)

    print("""
【测试因子】
1. 隔夜缺口因子 (gap_factor) - Rank IC: 0.0793
2. 缺口接受因子 (gap_acceptance) - Rank IC: 0.0744
3. 趋势质量因子 (trend_quality) - Rank IC: 0.0590
4. 有序趋势因子 (orderly_trend) - Rank IC: 0.0465
""")
    ...
    for symbol, results in all_results.items():
        ...
        for name, _, _ in test_configs:
            if name in results:
                ...
                if ret > original_ret and diff > 50:
                    print(f"{ret:>13.1f}%++", end="")
                elif ret > original_ret and diff > 0:
                    print(f"{ret:>13.1f}+", end="")
                elif ret < original_ret and diff < -50:
                    print(f"{ret:>13.1f}%--", end="")
                else:
                    print(f"{ret:>13.1f}%", end="")
            else:
    ...
    for symbol, results in all_results.items():
        best_name = max(results.keys(), key=lambda x: results[x]['total_return'])
        factor_wins[best_name] += 1

    print(f"\n{'因子':<20}{'胜出品种数':>10}")
    print("-" * 35)
    for name, wins in sorted(factor_wins.items(), key=lambda x: -x[1]):
        if wins > 0:
            print(f"{name:<20}{wins:>10}")

    # 推荐配置
    print("\n" + "=" * 140)
    print("各品种推荐因子配置".center(140))
    print("=" * 140)

    print(f"\n{'品种':<8}{'推荐因子':<20}{'收益':>12}{'回撤':>10}{'交易数':>8}{'胜率':>8}")
    print("-" * 70)

    for symbol, results in all_results.items():
        best_name = max(results.keys(), key=lambda x: results[x]['total_return'])
        best = results[best_name]
        print(f"{symbol:<8}{best_name:<20}{best['total_return']:>11.1f}%{best['max_drawdown']:>9.1f}%{best['total_trades']:>8}{best['win_rate']:>7.1f}%")
