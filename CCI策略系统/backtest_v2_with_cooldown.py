...
"""
历史数据回测系统 - V2+3天冷却版
核心思想：保留v2的狼性，增加v3的防弹衣
主要改进：
1. 保留v2所有激进开仓逻辑（牛市过滤 + YTD过滤 + STC/CCI）
2. 增加单一冷却机制：平仓后3天内不开新仓
3. 简单、实用、无look-ahead bias
"""
import pandas as pd
import numpy as np
...
def backtest_year(df_all, year, symbol):
    ...
    print(f"      冷却机制: 平仓后{params['cooldown_days']}天不开新仓")
    ...
    for i in range(1, len(df_year)):
        ...
        # 开仓逻辑（保留v2的所有激进条件，只增加冷却）
        if position == 0:
            # 冷却机制检查（修复：必须放在开仓逻辑内部！）
            if last_exit_date:
...
def main():
    print("=" * 100)
    print("历史数据回测系统 - V2+3天冷却版")
    print("=" * 100)
    print(f"运行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 100)
    print("\n核心改进:")
    print("  1. 保留v2所有激进开仓逻辑（狼性）")
    print("  2. 增加v3的防弹衣：平仓后3天不开仓")
    print("  3. 简单、实用、无look-ahead bias")
    print("=" * 100)
