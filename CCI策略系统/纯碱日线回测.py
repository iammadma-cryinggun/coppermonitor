...
def main():
    ...
    if result:
        ...
        print("\n与老参数小时数据对比(-25.83%):")
        print(f"  小时: -25.83%")
        print(f"  日线: {result['total_return']:.2f}%")
        print(f"  提升: {result['total_return'] - (-25.83):.2f}%")

    print("\n" + "=" * 100)
    print("结论")
    print("=" * 100)
    if result['total_return'] > -10:
        print("✓ 日线数据有效，建议实盘使用日线信号")
        print("  原因：日线过滤了日内噪音，信号更可靠")
    else:
        print("✗ 日线数据仍亏损")
        print("  建议：1) 检查数据质量 2) 策略可能已失效 3) 需重新设计")
