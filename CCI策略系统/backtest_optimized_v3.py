...
STRATEGY_PARAMS = {
    'SNIPER': {
        ...
        'trend_lookback': 10,  # 趋势回看周期（天）
        'max_trend_threshold': 0.05,  # 最大短期涨幅阈值（5%）
    },
    'TREND': {
        'stop_loss': 2.0,
        'stc_level': 60,
...
def backtest_year(df_all, year, symbol):
    ...
    for i in range(1, len(df_year)):
        ...
        if position != 0:
            ...
            # 优化4：分批止盈（SNIPER模式）
            if regime == 'SNIPER' and position == -1:
            ...
            if exit:
                remaining_size = position_size if regime == 'SNIPER' else 20
...
def main():
    print("=" * 100)
    print("历史数据回测系统 - 完整优化版 v3".center(100))
    print("=" * 100)
    ...
    print("  6. 优化止盈逻辑（分批止盈：2 ATR平半仓，4 ATR全平）")
