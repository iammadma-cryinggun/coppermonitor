...
def backtest_year(df_year, symbol, year):
    """单年份回测"""
    print(f"\n正在回测: {year}年")
    print("-" * 80)

    # 准备数据
    df_year = prepare_data(df_year).dropna()
    ...
    for i in range(1, len(df_year)):
        ...
        if position == 0:
            ...
            # TREND模式
                ...
                # 做多
                if row['stc'] > params['stc_level'] and row['cci'] > params['cci_trigger'] and row['vol_ratio'] > 1.0 and row['close'] > row['ma60']:
                    position = 1
                    entry_price = row['close']
...
def backtest_by_year(symbol, start_year, end_year):
    ...
    for year in range(start_year, end_year + 1):
        ...
        result = backtest_year(df_year, symbol, year)
