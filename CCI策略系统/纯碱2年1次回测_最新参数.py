"""
纯碱(SA0) 2年1次回测 - 最新参数配置
包含：
1. 量能过滤（vol_threshold: 0.50）
2. 隔夜缺口因子（gap_threshold: -0.15）
3. CCI超买平仓（去掉20%固定止盈）
4. 止损、死叉平仓
"""
import pandas as pd
...
def print_results(result, params):
    ...
    print("\n" + "=" * 100)
    print("纯碱(SA0) 2年1次回测结果".center(100))
    print("=" * 100)
    ...
    print("=" * 100)
    print(f"  隔夜缺口因子: 启用")
    print(f"  缺口阈值:     {params['gap_threshold']}")
    print(f"  量能过滤:      vol_ratio >= {params['vol_threshold']}")
...
def main():
    print("=" * 100)
    print("纯碱(SA0) 2年1次回测 - 最新参数配置".center(100))
    print("=" * 100)
    ...
    if result:
        ...
        # 保存详细交易记录
        output_file = 'D:\\期货数据\\铜期货监控\\CCI策略系统\\纯碱回测详细记录.csv'
