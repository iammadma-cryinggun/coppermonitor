...
def prepare_data(df):
    ...
    mad = tp.rolling(window=14).apply(lambda x: pd.Series(x).mad())
...
def run_dynamic_backtest(df, symbol):
    ...
    for i in range(1, len(df)):
        ...
        elif row['trend_strength'] > 2.0 and row['is_bullish']:
            current_mode = 'TREND'  # 趋势
            elif row['trend_strength'] > 2.0 and not row['is_bullish']:
            current_mode = 'TREND'  # 趋势
        else:
        ...
        if position != 0:
            ...
            if exit:
                ...
                print(f"    [平仓] {df.index[i]} {entry_mode} | {exit_reason} | 盈亏:{net_pnl:.0f}")
                position = 0
                entry_mode = ''
                continue
        ...
        if position == 0:
            ...
            if current_mode == 'SNIPER':
                params_sniper = STRATEGY_REPO['SNIPER']

                # 做空
                if params_sniper.get('long_enabled', False) and row['cci'] < params_sniper['cci_trigger']:
                    cond1 = row['stc'] < params_sniper['stc_level']  # STC回落
                    cond2 = row['cci'] > params_sniper['cci_trigger'] - 10  # CCI刚跌破
                ...
                # 做多
                if params_sniper.get('short_enabled', False) and row['cci'] > -params_sniper['cci_trigger']:
                    cond1 = row['stc'] > -params_sniper['stc_level']  # STC突破
                    cond2 = row['cci'] < params_sniper['cci_trigger'] + 10  # CCI刚突破
                    cond3 = row['vol_ratio'] > 0.5
            ...
            # TREND模式
                ...
                # 做多
                if params_trend.get('long_enabled', False) and row['cci'] > -params_trend['cci_trigger']:
                    cond1 = row['stc'] > -params_trend['stc_level']  # STC低位突破
                    cond2 = row['cci'] < params_trend['cci_trigger'] + 10  # CCI刚突破
                    cond3 = row['vol_ratio'] > 1.0
                    cond4 = row['close'] > row['open']  # 阳线
                ...
                # 做空
                if params_trend.get('short_enabled', False) and row['cci'] < -params_trend['cci_trigger']:
                    cond1 = row['stc'] > params_trend['stc_level']  # STC高位回落
                    cond2 = row['cci'] > params_trend['cci_trigger'] - 10  # CCI刚回落
                    cond3 = row['vol_ratio'] > 1.0
                    cond4 = row['close'] < row['open']  # 阴线
...
def backtest_by_year(symbol, start_year, end_year):
    ...
    if yearly_results:
        ...
        for _, r in results_df.iterrows():
            print(f"  {r['year']:<8} {r['total_return']:>9.2f}%   {r['total_trades']:>6}笔   {r['win_rate']:>7.1f}%   {r['mode']:<8}")
