# -*- coding: utf-8 -*-
"""
冰火双刀流 - 复合策略 (Composite Strategy)
核心思想：资产配置理论 (Portfolio Theory)
机制：
1. 将资金分为两份 (各50%)
2. 激进轨(V2.2)：止损冷3天，止盈冷0天 → 负责在暴跌中赚取暴利
3. 保守轨(V2.1)：无论盈亏，强制冷3天 → 负责在震荡中保护本金
结果：平滑资金曲线，穿越牛熊
"""
import pandas as pd
import numpy as np
...
CONFIG = {
    ...
    # --- 差异化参数 ---
    'allocation_ratio': 0.5,  # 资金分配比例 (50%)
...
def backtest_year_composite(df_all, year, symbol):
    """单年份回测 - 冰火双刀流"""
    print(f"\n{'='*80}")
    print(f"年份回测: {year}年 (冰火双刀流 - 复合策略)")
    print('-'*80)
    ...
    for i in range(1, len(df_year)):
        ...
        if pos_agg != 0:
            ...
            if not exit:
                ...
                if pos_agg == -1:
                    potential_loss = (row['close'] - entry_price_agg) * 20
                else:
                    potential_loss = (entry_price_agg - row['close']) * 20
        ...
        if pos_def != 0:
            ...
            if not exit:
                ...
                if pos_def == -1:
                    potential_loss = (row['close'] - entry_price_def) * 20
                else:
                    potential_loss = (entry_price_def - row['close']) * 20
...
def backtest_by_year(symbol, start_year, end_year):
    """按年份回测"""
    print(f"\n{'='*100}")
    print(f"年份回测: {symbol} ({start_year}-{end_year}) - 冰火双刀流")
    print('='*100)
    ...
    # 输出总结
    print(f"\n{'='*100}")
    print(f"年份回测总结: {symbol} ({start_year}-{end_year}) - 冰火双刀流")
    print('='*100)
...
def main():
    print("=" * 100)
    print("冰火双刀流 - 复合策略 (Composite Strategy)")
    print("=" * 100)
