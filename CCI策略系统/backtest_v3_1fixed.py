...
STRATEGY_PARAMS = {
    'SNIPER': {
        ...
        'partial_close_atr': 2.5,  # 分批止盈ATR阈值（提高到2.5）
        'full_close_atr': 4.5,  # 全部止盈ATR阈值（提高到4.5）
    },
    'TREND': {
        'stop_loss': 2.0,
        'stc_level': 60,
...
def check_short_term_trend(df_year, current_index, lookback_days=10):
    """检查短期趋势（过去N天的涨跌幅）"""
    if current_index < lookback_days:
        return 0

    start_price = df_year['close'].iloc[current_index - lookback_days]
    current_price = df_year['close'].iloc[current_index]
    short_trend = (current_price - start_price) / start_price
    return short_trend
...
def backtest_year(df_all, year, symbol):
    ...
    for i in range(1, len(df_year)):
        ...
        # 计算短期趋势
        short_term_trend = check_short_term_trend(
            df_year, i,
            params.get('trend_lookback', 10)
        )
        ...
        if position != 0:
            ...
            # 关键修复：使用价格变动计算盈亏，而不是profit_raw
            if position == -1:  # 做空
                price_change = entry_price - row['close']  # 正数=盈利，负数=亏损
            else:  # 做多
                price_change = row['close'] - entry_price  # 正数=盈利，负数=亏损
            ...
            if regime == 'SNIPER' and position == -1 and position_size == 20:
                ...
                # 盈利超过partial_threshold ATR，平一半仓位
                if profit_atr > partial_threshold:
                    close_size = position_size // 2  # 平10手
                    ...
                    print(f"    [平半仓] {current_date} 价格:{row['close']:.2f} 盈利:{price_change*close_size:.2f} ({profit_atr:.1f}ATR)")

                    position_size = position_size - close_size  # 剩余10手
                    # 不更新position，继续持有

                # 盈利超过full_threshold ATR，全部平仓
            ...
            if not exit:
                ...
                if price_change < -stop_loss_distance:  # 亏损超过止损距离
                    exit = True
        ...
        if position == 0:
            ...
            if last_exit_date:
                ...
                if days_since_last_exit < params.get('min_trade_interval', 5):
                    continue  # 距离上次平仓不足最小间隔，不开仓
            ...
            if regime == 'SNIPER':
                ...
                # 短期趋势确认
                if short_term_trend > params.get('max_trend_threshold', 0.05):
                    continue  # 过去N天涨幅过大，不做空
                ...
                if (current_stc < params['stc_level'] and
                    ...
                    print(f"    [开空] {current_date} 价格:{entry_price:.2f} YTD:{ytd_return:+.1f}% 短期:{short_term_trend*100:+.1f}%")
            ...
            if regime == 'TREND':
                if (current_stc > params['stc_level'] and
                    ...
                    entry_price = row['close']
                    entry_date = current_date
                    print(f"    [开多] {current_date} 价格:{entry_price:.2f}")
...
def main():
    print("=" * 100)
    print("历史数据回测系统 - v3.1修复版".center(100))
    print("=" * 100)
    ...
    print("  5. 增加交易间隔限制（最少5天）")
    print("  6. **修复分批止盈逻辑Bug**")
    print("  7. 增加短期趋势确认（过去10天涨幅检查）")
    print("=" * 100)
