...
def main():
    print("\n" + "="*80)
    print("铜FTT策略 - 原始版本（无STC止盈）".center(80))
    print("="*80)
    ...
    # 分析止盈方式
    stop_loss_count = len([t for t in result['trades'] if t['type'] == 'stop_loss'])
    trend_end_count = len([t for t in result['trades'] if t['type'] == 'trend_end'])

    print(f"\n离场方式统计:")
    print(f"  硬止损:     {stop_loss_count}笔")
    print(f"  趋势结束:   {trend_end_count}笔")
