...
def main():
    ...
    csv_file = Path('futures_data_4h/PTA_4hour.csv')
    ...
    print(f"\n对比分析：")
    print(f"  8%保证金: +167.47% (13笔)")
    print(f"  7%保证金: {result['return_pct']:+.2f}% ({result['trades']}笔)")
    diff = result['return_pct'] - 167.47
    print(f"  差异: {diff:+.2f}% ({(diff/167.47*100):+.1f}%)")
    ...
    if len(trades_df) > 0:
        ...
        print(f"  保证金/手: {margin_per_contract:,.2f} 元 (7%)")
        ...
        print(f"\n对比8%保证金:")
        margin_per_contract_8pct = contract_value * 0.08
        total_margin_8pct = margin_per_contract_8pct * contracts
        margin_usage_8pct = total_margin_8pct / capital_at_entry * 100
        print(f"  保证金/手: {margin_per_contract_8pct:,.2f} 元 (8%)")
        print(f"  总保证金: {total_margin_8pct:,.2f} 元")
        print(f"  保证金使用率: {margin_usage_8pct:.2f}%")
